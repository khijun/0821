...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (com.equals("/list") || com.equals("/")) {
			request.setAttribute("searchOption", false);
			...
			if(searchBy.length()>0||searchKeyword.length()>0) {
				request.setAttribute("searchOption", true);
				request.setAttribute("searchBy", searchBy);
				request.setAttribute("searchKeyword", searchKeyword);
				int pageNo = (tmp != null && tmp.length() > 0) ? Integer.parseInt(tmp) : 1;
				request.setAttribute("msgList", new BoardService().getMsgList(pageNo, searchBy, searchKeyword));
				request.setAttribute("pgnList", new BoardService().getPagination(pageNo, searchBy, searchKeyword));
			}
			view = "list.jsp";
